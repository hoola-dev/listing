...
<head>
	...
	@foreach(getSupportedLanguages() as $localeCode => $properties)
		@if (strtolower($localeCode) == strtolower(config('lang.abbr', config('app.locales'))))
			<link rel="canonical" href="{{ url($localeCode) }}"/>
		@else
			<link rel="alternate" href="{{ url($localeCode) }}" hreflang="{{ strtolower($localeCode) }}"/>
		@endif
	@endforeach
	@if (isset($post))
