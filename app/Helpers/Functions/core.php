...
function appIsInstalled()
{
	...
	try {
		...
		$modelFiles = array_filter(glob(app_path('Models') . DIRECTORY_SEPARATOR . '*.php'), 'is_file');
		
		if (is_array($modelFiles) && count($modelFiles) > 0) {
			foreach ($modelFiles as $filePath) {
				$table = getModelTableName($filePath);
 ...
 */
function getModelTableName($fileFullPath, $tablesPrefix = null)
{
